...
function defaults(){ return ['updated' => '', 'images' => new stdClass(), 'text' => new stdClass(), 'themes' => [], 'hours' => new stdClass(), 'items' => []]; }
...
if ($method === 'POST') {
  ...
  $themes = (isset($j['themes']) && is_array($j['themes'])) ? array_slice($j['themes'], 0, 9) : [];
  $hours  = (isset($j['hours'])  && is_array($j['hours'])  && count($j['hours']))  ? $j['hours']  : new stdClass();
  $items  = (isset($j['items'])  && is_array($j['items']))  ? array_slice($j['items'], 0, 50) : [];
  $clean = ['updated' => date('c'), 'images' => $images, 'text' => $text, 'themes' => $themes, 'hours' => $hours, 'items' => $items];
